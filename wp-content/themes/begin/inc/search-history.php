...
if ( zm_get_option( 'search_history' ) ) {
	...
	function be_hot_search( $limit = 10, $title = false ) {
		...
		if ( $title ) {
			$html .= '<h3 class="searches-title recently-searches-title"><i class="be be-favoriteoutline"></i>' . esc_html( $title ) . '</h3>';
		}
	...
	function load_hot_search_list() {
		...
		$html = '<div class="recently-searches">';
		$html .= '<h3 class="searches-title recently-searches-title"><i class="be be-favoriteoutline"></i>' . sprintf(__( '热门搜索', 'begin' ) ) . '</h3>';
