...
class WPJAM_Posts_Per_Page extends WPJAM_Option_Model{
	...
	public static function get_fields(){
		...
		$fields['posts_per_page']	= $number_field+['title'=>'全局',	'value'=>get_option('posts_per_page'),	'description'=>'博客全局设置的文章列表数量，空或者0则使用全局设置'];

		foreach(['author'=>'作者页','search'=>'搜索页','archive'=>'存档页'] as $key => $title){
			$other_fields[$key]	= $number_field+['title'=>$title];
		}
			...
			foreach(wp_list_sort($tax_objects, 'hierarchical', 'DESC', true) as $taxonomy => $tax_object){
				...
				if($tax_object->hierarchical){
					$tax_fields[$taxonomy.'_set']	= ['title'=>$title,	'type'=>'fields',	'fields'=>[
						...
						$taxonomy.'_individual'	=> ['type'=>'checkbox',	'description'=>'每个'.$title.'可独立设置数量',	'show_if'=>['key'=>$taxonomy, 'compare'=>'!=', 'value'=>'']]
					]];
				}else{
