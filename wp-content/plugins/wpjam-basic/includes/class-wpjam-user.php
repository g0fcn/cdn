...
class WPJAM_User{
	...
	public function __get($key){
		if(in_array($key, ['id', 'user_id'])){
			...
			if(in_array($key, ['user', 'data'])){
				return $user;
			}

			return $user->$key ?? $this->meta_get($key);
		}
	}
...
class WPJAM_User_Signup extends WPJAM_Register{
	...
	public function __call($method, $args){
		...
		if(str_ends_with($method, '_openid') || $method == 'get_by_user_email'){
	...
	public function get_attr($action='login', $form=''){
		...
		$fields['type']	= ['type'=>'hidden', 'value'=>$this->name];
