...
class AlipaySubmit {
	...
	function __construct($alipay_config){
		...
		$this->alipay_gateway_json = $this->alipay_config['apiurl'].'mapi.php';
	}
	...
	function buildRequestForm($para_temp, $method='POST', $button_name='正在跳转') {
		...
		while (list ($key, $val) = $this->replace_each ($para)) {
     ...
     * @return 返回的json转换数组
     */
	function buildRequestJson($para_temp, $method='POST') {
		//待请求参数数组
		$para = $this->buildRequestPara($para_temp);

		/*$result = wp_remote_request($this->alipay_gateway_json, array("method" => "POST", "body"=>$para));
		if($result){
	        $resultArray = json_decode($result['body'],true);
	    }else{
	    	$resultArray['msg'] = '请求失败，请稍后重试！';
	    }*/

	    $result = $this->http_post($this->alipay_gateway_json, $para);
	    if($result){
