...
function erphpdown_metaboxs() {
	...
	$meta_boxes = array(
		...
		array(
			"name"             => "member_down",
			"title"            => "VIP优惠",
			"desc"             => "专享指VIP免费、普通用户无法购买，专享购买指仅VIP可购买；高等级享有低等级的权限；没有说明VIP时长名称的优惠指所有VIP的优惠；可通过短代码<code>[vip]隐藏内容[/vip]</code>隐藏VIP专享内容，<code>[vip type=7]隐藏内容[/vip]</code>type的值默认可不填，若填7、8、9、10分别表示包月、包季、包年、终身",
			"type"             => "vipradio",
			'options' => array(
				'1' => '无',
	            '4' => '专享',
	...
	$meta_boxes[] = array(
		"name"             => "down_price",
		"title"            => "收费价格",
		"desc"             => "除<b>VIP专享</b>外，其他必须大于0（<b>VIP专享购买</b>也必须大于0），否则视为全网免费资源；免登录模式时单位为元",
		"type"             => "erphpnumber",
...
?>
	...
	.down-urls .down-url{margin-bottom: 5px;border: 1px dashed #ccc;padding: 5px;border-radius: 5px;position: relative;cursor: move;}
...
function erphpdown_show_erphptextareas( $args = array(), $value = false ) {
	...
	<div class="erphpdown-metabox-item erphpdown-metabox-item-pricetype1">
		<label class="title"><?php echo $title; ?></label>
		<div class="down-urls erphpdown-urls">
			<?php 
				if($value){
            		...
            		if($cnt){
            			for($i=0; $i<$cnt;$i++){
            				echo '<div class="down-url">
								<input type="number" min="1" step="1" name="down_urls[index][]" value="'.$value['index'][$i].'" placeholder="序号" style="width:8%" required><input type="text" name="down_urls[name][]" value="'.$value['name'][$i].'" placeholder="名称" style="width: 15%" required><input type="number" step="0.01" name="down_urls[price][]" value="'.$value['price'][$i].'" placeholder="价格" style="width: 10%"><div style="display: inline-block;width: 50%;"><textarea name="down_urls[url][]" placeholder="地址" style="width: 100%" rows="2">'.$value['url'][$i].'</textarea><a href="javascript:;" class="erphp-add-url button">上传媒体库文件</a> <a href="javascript:;" class="erphp-add-url2 button button-primary">上传本地文件</a> <span></span></div><select name="down_urls[vip][]" style="display:inline-block;"><option value="0">VIP默认优惠</option><option value="1" '.($value['vip'][$i] == '1'?'selected':'').'>无</option><option value="4" '.($value['vip'][$i] == '4'?'selected':'').'>VIP专享</option><option value="15" '.($value['vip'][$i] == '15'?'selected':'').'>包季VIP专享</option><option value="8" '.($value['vip'][$i] == '8'?'selected':'').'>年费VIP专享</option><option value="9" '.($value['vip'][$i] == '9'?'selected':'').'>终身VIP专享</option><option value="3" '.($value['vip'][$i] == '3'?'selected':'').'>VIP免费</option><option value="16" '.($value['vip'][$i] == '16'?'selected':'').'>包季VIP免费</option><option value="6" '.($value['vip'][$i] == '6'?'selected':'').'>年费VIP免费</option><option value="7" '.($value['vip'][$i] == '7'?'selected':'').'>终身VIP免费</option><option value="2" '.($value['vip'][$i] == '2'?'selected':'').'>VIP 5折</option><option value="5" '.($value['vip'][$i] == '5'?'selected':'').'>VIP 8折</option><option value="13" '.($value['vip'][$i] == '13'?'selected':'').'>VIP 5折|终身免费</option><option value="14" '.($value['vip'][$i] == '14'?'selected':'').'>VIP 8折|终身免费</option><option value="10" '.($value['vip'][$i] == '10'?'selected':'').'>VIP专享购买</option><option value="17" '.($value['vip'][$i] == '17'?'selected':'').'>包季VIP专享购买</option><option value="18" '.($value['vip'][$i] == '18'?'selected':'').'>包年VIP专享购买</option><option value="19" '.($value['vip'][$i] == '19'?'selected':'').'>终身VIP专享购买</option><option value="11" '.($value['vip'][$i] == '11'?'selected':'').'>专享购买|年费5折</option><option value="12" '.($value['vip'][$i] == '12'?'selected':'').'>专享购买|年费8折</option></select> <a href="javascript:;" class="del-down-url">删除</a>
							</div>';
            			}
            		}
				}
...
function erphpdown_show_downradio( $args = array(), $value = false ) {
	...
	<div class="erphpdown-metabox-item">
		...
		<?php if(!get_option('erphp_wppay_close')){?>
		<input type="radio" name="erphp_down" id="erphp_down5" <?php if($default == '5') echo 'checked'?> value="5" class="nologin noprice"/><label for="erphp_down5">免登录（旧）</label> &nbsp;
		<?php }?>
		...
		<?php if(!get_option('erphp_wppay_close')){?>
		<input type="radio" name="erphp_down" id="erphp_down5" <?php if($value5 == 'yes') echo 'checked'?> value="5" class="nologin noprice"/><label for="erphp_down5">免登录（旧）</label> &nbsp;
		<?php }?>
